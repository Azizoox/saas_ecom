...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark sticky-top mb-0 pb-0">
        <div class="container">
            <a class="navbar-brand d-flex align-items-center" href="<?php echo e(route('shop.index', ['subdomain' => $shop->subdomain ?? ''])); ?>">
                <?php if($shop && $shop->logo): ?>
                    <img src="<?php echo e(\Illuminate\Support\Facades\Storage::url($shop->logo)); ?>" alt="<?php echo e($shop->name); ?>" style="height: 40px; margin-right: 10px;">
                <?php endif; ?>
                <?php echo e($shop->name ?? 'Ma Boutique'); ?>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto align-items-center">
                        ...
                        <div class="position-relative">
                            <?php
                                ...
                                $searchAction = \Illuminate\Support\Facades\Route::has('shop.search')
                                    ...
                                    : '#';
                            ?>
                            <form class="d-flex" id="searchForm" action="<?php echo e($searchAction); ?>" method="GET">
                                <input class="form-control me-2" type="search" name="q" id="searchInput" placeholder="Rechercher un produit..." aria-label="Search" value="<?php echo e(request('q', '')); ?>">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="<?php echo e(route('shop.index', ['subdomain' => $shop->subdomain ?? ''])); ?>">
                            <i class="bi bi-house-door me-1"></i> Accueil
                        </a>
                    </li>
                    <!-- <?php if($shop): ?>
                        <?php $__currentLoopData = $shop->pages()->where('is_active', true)->orderBy('order')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $page): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <li class="nav-item">
                            <a class="nav-link <?php echo e(request()->routeIs('shop.page') && $page->slug == (request()->route('slug') ?? '') ? 'active' : ''); ?>" 
                               href="<?php echo e(route('shop.page', ['subdomain' => $shop->subdomain, 'slug' => $page->slug])); ?>"><?php echo e($page->title); ?></a>
                        </li>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <?php endif; ?> --> 
                    
                    <li class="nav-item">
                        <a class="nav-link" href="<?php echo e(route('shop.cart', ['subdomain' => $shop->subdomain ?? request()->route('subdomain', '')])); ?>">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function performSearch(query) {
                ...
                const shopSubdomain = '<?php echo e($shop->subdomain ?? request()->route("subdomain", "")); ?>';
                const baseSearchUrl = '<?php echo e(\Illuminate\Support\Facades\Route::has("shop.search") ? route("shop.search", ["subdomain" => "___SUB___"]) : "#"); ?>';
