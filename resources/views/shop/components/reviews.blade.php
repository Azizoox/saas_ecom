...
<section class="rv-section">
    ...
    <div class="rv-container">
        ...
        <div class="rv-header">
            ...
            <h2 class="rv-title">{{ $content['title'] ?? 'Avis Clients' }}</h2>
            <div class="rv-meta">
                <div class="rv-stars-display">
                    @for($s = 1; $s <= 5; $s++)
                        <svg class="rv-star-svg {{ $s <= round($avgRating) ? 'rv-star-svg--filled' : '' }}" viewBox="0 0 24 24">
                            <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/>
                        </svg>
                    @endfor
                </div>
                <span class="rv-avg">{{ number_format($avgRating, 1) }}</span>
                <span class="rv-count">· {{ count($validReviews) }} avis</span>
            </div>
        </div>

        @if($style === 'slider')
        {{-- ══════════════ SLIDER MODE ══════════════ --}}
        <div class="rv-slider-root" id="rvSlider-{{ $id }}">
            <div class="rv-track-wrap">
                <div class="rv-track">
                    @foreach($validReviews as $i => $review)
                    <div class="rv-slide" role="group" aria-label="Avis {{ $i + 1 }}">
                        <div class="rv-card rv-card--center">
                            ...
                            <blockquote class="rv-text">{{ $review['text'] }}</blockquote>
                            <div class="rv-author">
                                <div class="rv-avatar">
                                    {{ strtoupper(substr($review['name'] ?? 'C', 0, 1)) }}
                                </div>
                                <div>
                                    <p class="rv-author-name">{{ $review['name'] ?? 'Client' }}</p>
                                    @if(!empty($review['date']))
                                        <p class="rv-author-date">{{ $review['date'] }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
            ...
            <div class="rv-controls">
                <button class="rv-btn rv-btn--prev" data-slider="rvSlider-{{ $id }}" aria-label="Précédent">
                ...
                <div class="rv-dots" data-dots="rvSlider-{{ $id }}">
                    @foreach($validReviews as $i => $review)
                        <button class="rv-dot {{ $i === 0 ? 'rv-dot--active' : '' }}" data-index="{{ $i }}" aria-label="Slide {{ $i+1 }}"></button>
                    @endforeach
                </div>
                <button class="rv-btn rv-btn--next" data-slider="rvSlider-{{ $id }}" aria-label="Suivant">
        ...
        <div class="rv-grid">
            ...
            <div class="rv-card rv-card--grid" style="--i:{{ $i }}">
                ...
                <blockquote class="rv-text rv-text--grid">{{ $review['text'] }}</blockquote>
