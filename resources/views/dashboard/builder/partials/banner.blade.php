    ...
    <div class="card-body">
        <div class="row g-3 mb-3">
            <div class="col-md-6">
                ...
                <input type="text" name="content[text_main]" class="form-control" value="{{ $content['text_main'] ?? '' }}" placeholder="Ex: PROMO -50% sur toute la collection !">
            </div>
            <div class="col-md-6">
                <label class="form-label fw-bold"><i class="bi bi-chat-left-text"></i> Texte secondaire</label>
                <input type="text" name="content[text_secondary]" class="form-control" value="{{ $content['text_secondary'] ?? '' }}" placeholder="Ex: Livraison gratuite dès 50€">
            </div>
        </div>
        <div class="row g-3">
            <div class="col-md-4">
                <label class="form-label fw-bold"><i class="bi bi-speedometer"></i> Vitesse de défilement (sec)</label>
                <input type="number" name="content[speed]" class="form-control" value="{{ $content['speed'] ?? 10 }}" min="5" max="30" step="1">
            ...
            <div class="col-md-4">
                ...
                <div class="input-group">
                    <input type="color" name="content[bg_color]" class="form-control form-control-color banner-color" value="{{ $content['bg_color'] ?? '#000000' }}" id="bannerColor-{{ $componentId }}">
                    <input type="text" class="form-control banner-color-hex" id="bannerColorHex-{{ $componentId }}" value="{{ $content['bg_color'] ?? '#000000' }}" readonly>
                </div>
            </div>
            <div class="col-md-4">
        ...
        <div class="mt-4">
            ...
            <div class="border rounded p-3 text-white text-center banner-preview" style="background-color: {{ $content['bg_color'] ?? '#000000' }}; overflow: hidden;">
                <div class="d-inline-block">
                    <span class="fw-bold me-3 preview-main">{{ $content['text_main'] ?? 'Texte principal' }}</span>
                    <span class="opacity-75 preview-secondary">{{ $content['text_secondary'] ?? 'Texte secondaire' }}</span>
                </div>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    const root = $('#bannerColor-{{ $componentId }}').closest('.card');
