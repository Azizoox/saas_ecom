<div class="component-builder-section">
    <div class="card border-0 shadow-sm">
        ...
        <div class="card-body">
                    ...
                    <div class="col-md-3">
                        <div class="form-check form-switch p-3 bg-light rounded">
                            <input class="form-check-input" type="checkbox" name="content[options][autoplay]" 
                                   id="autoplay-{{ $componentId }}" value="1" 
                                   {{ ($content['options']['autoplay'] ?? true) ? 'checked' : '' }}>
                            <label class="form-check-label fw-bold" for="autoplay-{{ $componentId }}">
                    ...
                    <div class="col-md-3">
                        <div class="form-check form-switch p-3 bg-light rounded">
                            <input class="form-check-input" type="checkbox" name="content[options][loop]" 
                                   id="loop-{{ $componentId }}" value="1" 
                                   {{ ($content['options']['loop'] ?? true) ? 'checked' : '' }}>
                            <label class="form-check-label fw-bold" for="loop-{{ $componentId }}">
                    ...
                    <div class="col-md-3">
                        <div class="form-check form-switch p-3 bg-light rounded">
                            <input class="form-check-input" type="checkbox" name="content[options][pagination]" 
                                   id="pagination-{{ $componentId }}" value="1" 
                                   {{ ($content['options']['pagination'] ?? true) ? 'checked' : '' }}>
                            <label class="form-check-label fw-bold" for="pagination-{{ $componentId }}">
                    ...
                    <div class="col-md-3">
                        <div class="form-check form-switch p-3 bg-light rounded">
                            <input class="form-check-input" type="checkbox" name="content[options][arrows]" 
                                   id="arrows-{{ $componentId }}" value="1" 
                                   {{ ($content['options']['arrows'] ?? true) ? 'checked' : '' }}>
                            <label class="form-check-label fw-bold" for="arrows-{{ $componentId }}">
                ...
                <div class="row g-3 mt-3">
                    <div class="col-md-4">
                        ...
                        <input type="number" name="content[options][speed]" class="form-control" 
                               value="{{ $content['options']['speed'] ?? 3000 }}" min="1000" max="10000" step="500">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="content[options][transition]" class="form-control" 
                               value="{{ $content['options']['transition'] ?? 500 }}" min="200" max="2000" step="100">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="content[options][height]" class="form-select">
                            <option value="small" {{ ($content['options']['height'] ?? '') === 'small' ? 'selected' : '' }}>Petite (300px)</option>
                            <option value="medium" {{ ($content['options']['height'] ?? 'medium') === 'medium' ? 'selected' : '' }}>Moyenne (500px)</option>
                            <option value="large" {{ ($content['options']['height'] ?? '') === 'large' ? 'selected' : '' }}>Grande (700px)</option>
                            <option value="full" {{ ($content['options']['height'] ?? '') === 'full' ? 'selected' : '' }}>Plein écran</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <!-- Slides -->
            <div class="slides-container builder-slider" data-component-id="{{ $componentId }}">
                <div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">
                    <h6 class="fw-bold text-primary mb-0">
                        <i class="bi bi-collection"></i> Slides 
                        <span class="badge bg-primary slides-count">{{ count($content['slides'] ?? []) ?: 3 }}</span>
                    </h6>
                ...
                <div class="slides-list sortable-list">
                    ...
                    <div class="card mb-3 border-primary slide-item" data-index="{{ $i }}">
                        <div class="card-header bg-primary bg-opacity-10 d-flex justify-content-between align-items-center">
                            <span class="fw-bold">
                                <i class="bi bi-grip-vertical handle cursor-move"></i> 
                                Slide #<span class="slide-number">{{ $i + 1 }}</span>
                            </span>
                            <div class="btn-group">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="image-preview-container mb-2 position-relative">
                                        @if(isset($content['slides'][$i]['image']))
                                            <img src="{{ Storage::url($content['slides'][$i]['image']) }}" 
                                    ...
                                    <input type="file" name="content[slides][{{ $i }}][image]" 
                                           class="form-control slide-image-input" accept="image/*">
                                    <input type="hidden" name="content[slides][{{ $i }}][existing_image]" 
                                           value="{{ $content['slides'][$i]['image'] ?? '' }}">
                                </div>
                                ...
                                <div class="col-md-8">
                                    <div class="row g-3">
                                        <div class="col-12">
                                            ...
                                            <input type="text" name="content[slides][{{ $i }}][title]" 
                                                   class="form-control form-control-lg" 
                                                   value="{{ $content['slides'][$i]['title'] ?? '' }}" 
                                        ...
                                        <div class="col-12">
                                            ...
                                            <textarea name="content[slides][{{ $i }}][description]" 
                                                      class="form-control" rows="3" 
                                                      placeholder="Découvrez notre nouvelle collection avec jusqu'à -50% de réduction">{{ $content['slides'][$i]['description'] ?? '' }}</textarea>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" name="content[slides][{{ $i }}][button_text]" 
                                                   class="form-control" 
                                                   value="{{ $content['slides'][$i]['button_text'] ?? '' }}" 
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="url" name="content[slides][{{ $i }}][link]" 
                                                   class="form-control" 
                                                   value="{{ $content['slides'][$i]['link'] ?? '' }}" 
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select name="content[slides][{{ $i }}][text_position]" class="form-select">
                                                <option value="left" {{ ($content['slides'][$i]['text_position'] ?? 'left') === 'left' ? 'selected' : '' }}>Gauche</option>
                                                <option value="center" {{ ($content['slides'][$i]['text_position'] ?? '') === 'center' ? 'selected' : '' }}>Centre</option>
                                                <option value="right" {{ ($content['slides'][$i]['text_position'] ?? '') === 'right' ? 'selected' : '' }}>Droite</option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select name="content[slides][{{ $i }}][overlay]" class="form-select">
                                                <option value="none" {{ ($content['slides'][$i]['overlay'] ?? 'none') === 'none' ? 'selected' : '' }}>Aucun</option>
                                                <option value="light" {{ ($content['slides'][$i]['overlay'] ?? '') === 'light' ? 'selected' : '' }}>Léger</option>
                                                <option value="medium" {{ ($content['slides'][$i]['overlay'] ?? '') === 'medium' ? 'selected' : '' }}>Moyen</option>
                                                <option value="dark" {{ ($content['slides'][$i]['overlay'] ?? '') === 'dark' ? 'selected' : '' }}>Sombre</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
...
$(document).ready(function() {
    const root = $('.builder-slider[data-component-id="{{ $componentId }}"]');
