...
@section('title', 'Builder — Page d\'accueil')
                ...
                <div class="canvas-header">
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-2">
                        @if(isset($shops) && $shops->count() > 1)
                            <form method="GET" action="{{ route('builder.index') }}" class="shop-select-wrap">
                        ...
                        @endif
                        <a href="{{ route('shop.index', ['subdomain' => $shop->subdomain]) }}" target="_blank" class="btn btn-outline-primary btn-preview">
                ...
                @else
                    <div id="components-list">
                        @foreach($components as $component)
                            <div class="component-block component-item" data-id="{{ $component->id }}">
                                <div class="block-header">
                                    <span class="block-handle" title="Glisser pour réorganiser"><i class="bi bi-grip-vertical"></i></span>
                                    <span class="block-type-badge bg-{{ $component->type === 'slider' ? 'primary' : ($component->type === 'categories' ? 'success' : ($component->type === 'premium_categories' ? 'warning' : ($component->type === 'banner' ? 'info' : 'secondary'))) }} bg-opacity-10 text-{{ $component->type === 'slider' ? 'primary' : ($component->type === 'categories' ? 'success' : ($component->type === 'premium_categories' ? 'warning' : ($component->type === 'banner' ? 'info' : 'secondary'))) }}">
                                        {{ str_replace('_', ' ', $component->type) }}
                                    </span>
                                    ...
                                    <span class="block-summary">
                                        @switch($component->type)
                                            @case('slider') {{ count($component->content['slides'] ?? []) }} slide(s) @break
                                            @case('categories') Style : {{ $component->content['style'] ?? 'grid' }} @break
                                            @case('reviews') {{ count($component->content['reviews'] ?? []) }} avis @break
                                            @default Configuré
                                        @endswitch
                                    </span>
                                ...
                                <div class="block-form-wrap">
                                    <form action="{{ route('builder.update', $component) }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="form-check form-switch mb-4">
                                            <input class="form-check-input" type="checkbox" name="is_active" id="active-{{ $component->id }}" {{ $component->is_active ? 'checked' : '' }}>
                                            <label class="form-check-label fw-semibold" for="active-{{ $component->id }}">Bloc visible sur la page</label>
                                        </div>
...
(function() {
    ...
    if (modal) {
        modal.addEventListener('show.bs.modal', function(e) {
            ...
            if (shopEl) document.getElementById('shopIdInput').value = shopEl.value || '{{ $shop->id }}';
